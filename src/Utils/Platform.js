...
const provPlatformDebug = console.log;
...
class Platform {
  ...
  async platformName(name) {
    ...
    await this.#Database.Modify(
      false,
      "platform",
      { platformUrl: this.#platformUrl, clientId: this.#clientId },
      { platformName: name },
    );
    this.#platformName = name;
    return name;
  ...
  async platformActive(active) {
    if (active === undefined) {
      // Get platform status
      const platformStatus = await this.#Database.Get(false, "platformStatus", {
        id: this.#kid,
      });
  ...
  async platformPublicKey() {
    const key = await this.#Database.Get(this.#ENCRYPTIONKEY, "publickey", {
      kid: this.#kid,
    });
    return key[0].key;
  }
  ...
  async platformPrivateKey() {
    const key = await this.#Database.Get(this.#ENCRYPTIONKEY, "privatekey", {
      kid: this.#kid,
    });
    return key[0].key;
  }
  ...
  async platformAuthConfig(method, key) {
    ...
    if (
      ...
      throw new Error(
        'INVALID_METHOD. Details: Valid methods are "RSA_KEY", "JWK_KEY", "JWK_SET".',
      );
  ...
  async platformAuthorizationServer(authorizationServer) {
    if (!authorizationServer)
      return this.#authorizationServer || this.#accesstokenEndpoint;
    await this.#Database.Modify(
      false,
      "platform",
      { platformUrl: this.#platformUrl, clientId: this.#clientId },
  ...
  async platformJSON() {
    const platformJSON = {
      ...
      authorizationServer:
        this.#authorizationServer || this.#accesstokenEndpoint,
  ...
  async remove() {
    console.log(
      "Deprecation warning: The Platform.remove() method is now deprecated and will be removed in the 6.0 release. Use Platform.delete() instead.",
    );
    return this.delete();
  }
