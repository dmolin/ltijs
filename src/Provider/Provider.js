...
const { setupCustomLogger, Logger } = require("../Utils/Logger");
...
class Provider {
  ...
  #unregisteredPlatformCallback = async (req, res) => {
    provMainDebug(
      `Unregistered platform attempting connection: ${JSON.stringify(req.body)}`,
    );
    return res.status(400).send({
      status: 400,
      error: "Bad Request",
  ...
  setup(encryptionkey, database, options, logger) {
    ...
    if (logger) {
      ...
      log.info("LTI Provider setup: setting custom logger");
    }
    ...
    const sessionValidator = async (req, res, next) => {
      ...
      provMainDebug(
        `Path does not match reserved endpoints [${this.#loginRoute}, ${this.#keysetRoute}, ${this.#dynRegRoute}]`,
      );
        ...
        if (!ltik) {
          ...
          if (idtoken) {
            ...
            await this.Database.Replace(
              false,
              "contexttoken",
              { contextId: contextId, user: valid.sub },
              contextToken,
            );
    ...
    this.app.all(this.#loginRoute, async (req, res) => {
      ...
      try {
        ...
        log.info(
          "Provider: Receiving a login request from: " +
            iss +
            ", clientId: " +
            clientId,
        );
        ...
        if (platform) {
          ...
          provMainDebug(query);
          provMainDebug(
            `redirecting to ${await platform.platformAuthEndpoint()}`,
          );
          res.redirect(
            url.format({
    ...
    try {
      ...
      if (options && options.serverless) {
        ...
        if (!conf.silent) {
          ...
          if (!conf.silent) {
            const message =
              `LTI Provider will handle requests on the current server endpoints` +
